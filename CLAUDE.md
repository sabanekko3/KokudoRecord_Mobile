...
- 線のクリックは `routesAtPoint()` で全レイヤに自分で当たり判定を掛ける（重複区間で路線を選ぶため）。
  40m 相当をズームで画素に換算（6〜25px）
...
- `python tools/smoke.py` … http.server + ヘッドレス Edge。起動 → 記録追加 → タブ → CSV →
  道の駅 → 削除 → 再読み込みで記録が残る、まで通す
- `?debug` を付けて開くと進み具合が画面下に出る（スマホには開発者ツールが無い）。
  `?nosw` でサービスワーカーを登録しない
...
- 背景地図はオンラインのときだけ。海岸線と県境を同梱して canvas に描けばオフラインでも
  形が分かる（1〜2MB の見込み。未着手）
- 実機（Android / iPhone）での速さと使い勝手は未確認。全路線の線（lines.json 5MB）を
  L.polyline 459 本で描いているので、古い端末では重いかもしれない
